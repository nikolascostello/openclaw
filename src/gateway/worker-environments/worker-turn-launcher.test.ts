...
describe("worker turn launcher local placement", () => {
  ...
  it.each([
    ...
    {
      scenario: "failed execution and computer cleanup",
      returnedFailure: false,
      cleanupFails: true,
      executionFailure: "Codex paired execution device disconnected; start a fresh attempt",
      expectedError:
        "Codex paired execution device disconnected; start a fresh attempt\n" +
        "Computer cleanup also failed: computer close failed | native close failed\n\n" +
        "Workspace recovery also failed: workspace manifest memo exceeds its entry limit. " +
        "Remote changes may not have been applied locally. Resolve the workspace error, then retry.",
      expectedTerminalReason: "Codex paired execution device disconnected; start a fresh attempt",
    },
    ...[false, true].map((cleanupFails) => ({
      scenario: `returned failure with computer cleanup failure=${cleanupFails}`,
      returnedFailure: true,
      cleanupFails,
      executionFailure: "recorded primary failure",
      expectedError:
        ...
        "\n\nWorkspace recovery also failed: workspace manifest memo exceeds its entry limit. " +
        "Remote changes may not have been applied locally. Resolve the workspace error, then retry.",
      expectedTerminalReason: "recorded primary failure",
    })),
  ])(
    ...
    async ({
      ...
      const environments: WorkerTurnEnvironmentService = {
        ...
        prepareComputer: vi.fn(async () =>
          cleanupFails
            ? {
                descriptor: computerDescriptor("fixture-node"),
                bind: vi.fn(),
                close: async () => {
        ...
        .executeTurn(
          ...
          async () => {
            if (executionFailure) {
              if (returnedFailure) {
                return {
                  payloads: [{ text: executionFailure, isError: true }],
                  meta: {
                    durationMs: 1,
                    error: {
      ...
      const reconciliation =
        failure instanceof WorkerWorkspaceReconciliationError
          ? failure
          : failure instanceof Error
            ? failure.cause
            : undefined;
      expect(reconciliation).toBeInstanceOf(WorkerWorkspaceReconciliationError);
      if (!(reconciliation instanceof WorkerWorkspaceReconciliationError)) {
        throw new Error("Expected a typed reconciliation failure");
      }
      expect(reconciliation.cause).toBe(reconciliationError);
