...
An OpenClaw worker waits for desktop cleanup before acknowledging that its turn is finishing. If cleanup fails after the assistant has answered, the turn still reports failure with a bounded, redacted cleanup diagnostic; the answer remains in the transcript. A cleanup or result-validation failure does not undo earlier input and must not be treated as permission to replay it blindly.
