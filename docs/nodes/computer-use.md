...
Provider descriptors declare `contractVersion: 2`. Invalid capability descriptors or `computer.act` result envelopes are rejected with `COMPUTER_CONTRACT_MISMATCH`. New native input results use `{ok:true}`. Older paired native nodes, including v2026.8.1, may still return numeric `cursorX` and `cursorY`; the Gateway accepts either field independently because nodes upgrade separately. This typed receipt compatibility remains until an explicit versioned paired-node upgrade/removal contract. Unknown fields and invalid types remain rejected; `effect` is optional. Result validation happens after invocation, so a rejected result or missing `effect` is not proof that input was never posted.
