...
| `pruneAfter`            | `"30d"`               | archive eligible stale conversations; delete eligible disposable entries                    |
...
When combined physical usage exceeds `maxDiskBytes`, `mode: "enforce"` first reclaims checkpointable database space, then removes the oldest retained reset/delete archives. If usage is still above `highWaterBytes`, it walks historical SQLite windows by `session_windows.updated_at`, oldest first. Eligible history is not referenced by a live entry, route target, or admitted/in-flight run, and is not owned by an archived, pinned, or recently protected logical session. For each victim with transcript events, cleanup stores the compressed archive in the same write transaction that removes the window and its transcript, trajectory, active, index, and FTS projections. It publishes, syncs, and reads back the derived file after commit. Trajectory-only history can be reclaimed without creating an empty transcript archive. Cleanup rechecks references and logical-owner protection at deletion time, remeasures physical usage after each archive or history victim, and stops at `highWaterBytes` or when no eligible data remains. Protected history can keep usage above the target.
...
`maxEntries` defaults to 5,000 unarchived logical session rows and honors an explicit configured limit. Archived rows are outside both cap-pressure detection and overflow accounting. Age and count maintenance archive eligible durable conversations in place without exporting or deleting their transcripts. Main, pinned, model-locked, active or admitted sessions, and routable external conversations (including direct DMs, groups, and threads) are protected. Protected unarchived rows still count, so the active population can remain above the cap. Automatic archive skips active work rather than stopping it. Synthetic runtime entries (cron, hooks, heartbeat, ACP, sub-agents) retain their existing deletion rules. Isolated cron runs use a separate `cron.sessionRetention` control, independent of model-run probe retention.

`--dry-run` previews archive and deletion actions against the unarchived cap; `--enforce` applies that cleanup immediately without removing protection. Reports keep total rows in `beforeCount`/`afterCount` and expose unarchived rows in `beforeActiveCount`/`afterActiveCount`. `archived` counts in-place archives, while `pruned`/`capped` count deleted disposable rows. Restore an archived conversation from the Control UI to continue it; explicit deletion remains separate. See [Sessions CLI](/cli/sessions#cleanup-maintenance).

Normal Gateway writes flow through the session accessor, which serializes per-agent SQLite mutations through the runtime writer path. Runtime code should prefer the accessor helpers in `src/config/sessions/session-accessor.ts`; legacy `sessions.json` helpers are migration and offline-maintenance tools. When a Gateway is reachable, non-dry-run `openclaw sessions cleanup` and `openclaw agents delete` delegate store mutations to the Gateway so cleanup joins the same writer queue; `--store <path>` is the explicit offline repair path for a selected legacy store and always stays local (as does `--dry-run`). Runtime `maxEntries` cleanup uses a 10% high-water buffer for production-sized caps: the default trigger is 5,500 unarchived rows, targeting 5,000. Reads never prune or cap entries during Gateway startup - only writes or `openclaw sessions cleanup --enforce` do, and the latter applies the cap immediately and prunes old unreferenced legacy transcript, checkpoint, and trajectory artifacts even with no disk budget configured. Automatic archive commits metadata before managed-worktree cleanup; filesystem and Git work remain outside SQLite transactions, and restore recreates the checkout before reopening admission.
